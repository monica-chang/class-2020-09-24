...
sep_old <- 
  ...
  summarize(u_grad_old = sum(u_grad), .groups = "drop") %>% 
  filter(u_grad_old > 10)
...
sep_new <- 
  ...
  rename_with(~ str_replace(.x, "course_", "")) %>% 
  group_by(id, title, name, department) %>% 
  summarize(u_grad_new = sum(u_grad), .groups = "drop") %>% 
  filter(u_grad_new > 10)
```
...
```{r scene1a}
sep_join <- inner_join(sep_old, sep_new, by = c("id", "department", "name", "title")) %>%
  mutate(diff = u_grad_new - u_grad_old)
  
five_increase <- sep_join %>%
  arrange(desc(diff)) %>%
  head(5)
...
rbind(five_decrease, five_increase) %>%
  arrange(diff) %>%
  ggplot(aes(x = reorder(title, diff), y = diff)) + 
    geom_col() +
    coord_flip() +
    labs(title = "Courses with greatest increase and decrease in enrollment",
         x = "Course title",
         y = "Change in enrollment from 9/1 - 9/22")
